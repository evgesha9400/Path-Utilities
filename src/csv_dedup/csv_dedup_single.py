...
"""
CSV Single-Column Deduplication Script
Removes duplicate rows based on one selected column
"""
...
def analyze_single_duplicates(input_file, column_index):
    """Analyze single-column duplicates in CSV file"""
    ...
    print(f'   Deduplication column: "{column_name}" (column {column_index + 1})')
    print()
    ...
    for row_data, line_num in data:
        # Get the key value from the selected column
        if column_index < len(row_data):
            key_value = row_data[column_index].strip().lower()
        else:
            key_value = ""

        duplicate_groups[key_value].append((row_data, line_num))
    ...
    if len(duplicate_keys) == 0:
        ...
        print(f'   All {original_count:,} rows have unique values in "{column_name}".')
        return 0
    else:
        ...
        for key_value, rows in duplicate_groups.items():
...
def display_row(row, headers, line_num, column_index):
...
def manual_deduplication(input_file, column_index, output_file):
...
def auto_deduplication(input_file, column_index, strategy, output_file):
    ...
    for row_data, row_num in data:
        if column_index < len(row_data):
            ...
            key_value = ""
        duplicate_groups[key_value].append((row_data, row_num))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CSV Single-Column Deduplication - Remove duplicates based on one column"
    )
    parser.add_argument(
        "input_file",
        nargs="?",
    ...
    print("🔍 SINGLE-COLUMN DEDUPLICATION")
    print("════════════════════════════════════════════════════════════════")
    print("This script removes rows that have duplicate values in ONE selected column.")
    print("• Choose which column to use as the duplicate key (e.g., email, ID, SKU)")
    print("• Rows with the same value in that column are considered duplicates")
    ...
    print("Select the column to use for deduplication:")
    print("Rows with the same value in this column will be considered duplicates.")
    print()

    while True:
        try:
            column_choice = input(f"Column number (1-{num_cols}): ").strip()
            if column_choice.isdigit():
                choice_num = int(column_choice)
                if 1 <= choice_num <= num_cols:
                    column_index = choice_num - 1  # Convert to 0-based
                    break
    ...
    # Analyze duplicates
    duplicate_count = analyze_single_duplicates(input_file, column_index)
    print()
    ...
    if use_manual:
        output_file = f"{base_name}_single_manual_deduplicated.csv"
    else:
        output_file = f"{base_name}_single_{strategy}_deduplicated.csv"
    ...
    try:
        if use_manual:
            manual_deduplication(input_file, column_index, output_file)
        else:
            auto_deduplication(input_file, column_index, strategy, output_file)
    except KeyboardInterrupt:
        print("\n❌ Operation cancelled by user")
        sys.exit(1)

    print()
    print("🎉 Single-column deduplication completed successfully!")
